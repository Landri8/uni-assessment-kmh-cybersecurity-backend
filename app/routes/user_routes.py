...
def verify_recaptcha():
    try:
        ...
        try:
            send_email(to=cachedUser.get('email'), subject="Hi {name}, please verify your EMS account.".format(name=cachedUser.get('name')), code=verification_code)
            responseUserObject = {
                "name": cachedUser.get('name'),
                "email": cachedUser.get('email'),
...
def resend_email():
    try:
        ...
        try:
            send_email(to=email, subject="Please verify your EMS account.", code=verification_code)
